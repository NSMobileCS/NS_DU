...
class Dir:
    ...
    def __init__(self, path, rcr_dep=0, rcr_dep_max=16, dialog_active=False, isThread=False, verbose=False):
        ...
        except OSError:     #we catch the exception that comes up when we can't descend into a directory
                            #currently, non-traversable directories are ignored
            if self.verbose:
                print("\n"*3 + "!"*5 + "__OSError__" + "!" * 5 + "\n" * 2)
                print(self)
            shortout = True
    ...
    def dialog(self):
        """
        this is for interactive text mode (cmd line), but it never needs to be called directly.

        you should be able to run cmd line mode as follows:

            $ python3 -c "import nsduCore; nsduCore.txtuiLoop()"

            alternatively, without txtuiLoop wrapper function:

            $ python3 -i nsduCore.py
            >>> d = Dir('/home/example', dialog_active=True)
